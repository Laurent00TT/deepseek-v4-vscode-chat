...
- **[#22](https://github.com/Laurent00TT/deepseek-v4-vscode-chat/issues/22): peak/off-peak pricing hint in the status-bar tooltip.** DeepSeek moved V4 Flash/Pro to time-of-day billing on 2026-08-16 (peak 01:00–04:00 and 06:00–10:00 UTC, off-peak everywhere else). Hovering the balance now shows which side of the boundary the system clock is on and the **local** time of the next flip (`$(flame) Peak pricing · off-peak starts 18:00` / `$(check) Off-peak pricing · until 09:00` — flame = burning money, check = cheap window, go ahead). Only the window is hardcoded (`src/off_peak.ts`, one documented constant) — no prices or multipliers, consistent with the balance-diff cost philosophy: prices drift, and the real bill stays anchored to `/user/balance`. A one-shot timer re-armed at each window edge rebuilds the tooltip so an idle window can't show yesterday's state after sitting across a boundary (the tooltip is declarative — see the `flashRefreshAck` comment).
- **`test/unit_off_peak.mjs`** — pins the window constant and the half-open `[start, end)` edge convention second-by-second around all four edges, `nextBoundary`'s strictly-after semantics (including sub-minute precision, the evening wrap to next day, and a month rollover), and the structural invariants the boundary timer relies on: every edge flips the state, and the next boundary is always in the strict future within 24h.
