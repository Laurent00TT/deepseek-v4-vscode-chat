...
- **README (EN + zh-CN) rewritten for concision and newcomers.** About 40 % fewer lines and a quarter fewer words with the same facts: prerequisites and quick start first (including what to do when the models are hidden in the picker), a one-line definition of the `(thinking)` variants, a user-facing "What you get" list instead of an internals list, a short Images section, the native-provider rationale in two bullets (reasoning round-trip — now with the cache-price consequence spelled out — and real cost), compact command/setting tables, and FAQ entries trimmed to a few lines each. Wording corrected along the way: the command palette title is `Compact Copilot Chat` (no category prefix); session spend lives in the status-bar hover, not the bar; the cache-breakdown FAQ no longer asserts a cause the code cannot know; the 2026-08 reasoning-rule relaxation is stated as a dated live observation, not a policy. The bundled walkthrough's model table now lists the two Flash Vision variants (it still said "four variants"). Internals and history stay in ARCHITECTURE/CHANGELOG.
